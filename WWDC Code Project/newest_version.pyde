# ---------------------------------------------------------
# Monthly Habit Tracker | ADHD & Neurodivergent Children and Young Adults Prototype
...
# ASCII-only monthly quotes (avoid fancy punctuation mojibake at runtime);
# we add typographic quotes “ ” at draw time.
MONTH_QUOTES = [
  "",
...
# ------------------ LAYOUT ------------------
WIN_W, WIN_H = 1280, 820      # not fullscreen
SHEET_MARGIN = 36
...
MONTH_W, MONTH_H = 300, 36
MONTH_X = SHEET_X + SHEET_W - MONTH_W - 140
MONTH_Y = TITLE_Y - 2
PREV_X, PREV_Y = MONTH_X - 56, MONTH_Y
NEXT_X, NEXT_Y = MONTH_X + MONTH_W + 12, MONTH_Y

# Action buttons
BTN_W, BTN_H = 132, 32
ADD_X, ADD_Y = SHEET_X + 24, TITLE_Y + 52
DEL_X, DEL_Y = ADD_X + BTN_W + 12, ADD_Y

# Tabs under the title (fixing overlap)
TAB_H = 34
TAB_TRK_W, TAB_VIS_W = 150, 180
TAB_TRK_X, TAB_TRK_Y = SHEET_X + 24, TITLE_Y + 8
TAB_VIS_X, TAB_VIS_Y = TAB_TRK_X + TAB_TRK_W + 10, TAB_TRK_Y

# Header safe baseline (after tabs + buttons)
HEADER_BOTTOM = max(TAB_TRK_Y + TAB_H, ADD_Y + BTN_H) + 16

# Tracker grid & side panel
GRID_X, GRID_Y = SHEET_X + 72, HEADER_BOTTOM + 28
GRID_W, GRID_H = 820, 440
LEFT_W = 230

PANEL_X, PANEL_Y = GRID_X + GRID_W + 18, GRID_Y
PANEL_W, PANEL_H = SHEET_X + SHEET_W - PANEL_X - 24, GRID_H

# Quote card + padding
QUOTE_X, QUOTE_Y = PANEL_X + 14, PANEL_Y + PANEL_H - 122
QUOTE_W, QUOTE_H = PANEL_W - 28, 108

# Progress bar + reward
PROG_X, PROG_Y, PROG_W, PROG_H = GRID_X, GRID_Y + GRID_H + 16, (PANEL_X + PANEL_W) - GRID_X, 22
REWARD_X, REWARD_Y = GRID_X, PROG_Y + PROG_H + 18
REWARD_W, REWARD_H = PROG_W, 150

# Visualisation canvas and Save button
VIS_X, VIS_Y = SHEET_X + 60, HEADER_BOTTOM + 34
VIS_W, VIS_H = SHEET_W - 120, SHEET_H - (VIS_Y - SHEET_Y) - 40
SAVE_X, SAVE_Y, SAVE_W, SAVE_H = SHEET_X + SHEET_W - 140, TAB_TRK_Y, 116, TAB_H
...
QUOTE_BG  = C(230,210,255)  # pastel purple quote card
...
def draw_header():
    ...
    textFont(UI_FONT); textSize(16)
    textAlign(RIGHT, CENTER); text("MONTHS:", MONTH_X-14, MONTH_Y + MONTH_H/2)
...
def draw_top_buttons():
    draw_button(ADD_X, ADD_Y, BTN_W, BTN_H, "+ New Habit")
    draw_button(DEL_X, DEL_Y, BTN_W, BTN_H, "Delete Habit")
